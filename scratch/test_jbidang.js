...
(async () => {
    const { data: stdList } = await supabase.from('rpjmdes_standar').select('kode_unik_full, kode_unik, jenis_bidang');
    ...
    const { data: rabData } = await supabase.from('rab').select('id, kode_unik_full, kode_unik, jenis_bidang, nama_kegiatan, tahun').eq('tahun', 2027);
